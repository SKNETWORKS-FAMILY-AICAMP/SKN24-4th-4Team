...
| LLM | GPT-4o / GPT-4o-mini | 안전 필터·intent 분류·언어 감지는 mini로 비용 절감, 최종 답변 생성은 4o로 품질 확보 |
...
| 항목 | 내용 |
|------|------|
| 멀티턴 대화 맥락 유지 | `chat_history` 필드 추가 — 이전 대화 이력을 Intent Router에 전달해 follow-up 질문 처리 개선 |
| 언어 감지 개선 | 고유명사/보험 전문용어 제거 후 Unicode 스크립트 비율 기반 감지 — 한국어+영어 혼합 질문 오감지 방지 |
| RAG 응답 속도 개선 | ChromaDB 클라이언트를 모듈 레벨에서 캐싱 — 첫 요청 시 1회만 연결하고 이후 요청은 캐시된 클라이언트를 재사용해 반복 쿼리 속도 개선 *(TODO: RunPod 환경 실측 수치 추가)* |
| Intent Router 프롬프트 개선 | insurer 시스템 제공 명시, within_compare 플랜 미지정 허용, general_query vs recommendation 구분 강화 |
| 비교표 언어 강제 | 비교 기준이 영어여도 답변 내용은 사용자 언어로 출력 |
| 비교 페이지 일반 질문 처리 | 비교 페이지에서 일반 질문 입력 시 비교표 대신 일반 답변 반환 |

### 향후 개선점

| 항목 | 내용 |
|------|------|
| 보험사 확대 | 현재 4개 → 추가 민간 보험사 지원 |
| 개인화 | 사용자 플랜 정보 저장 후 맞춤 답변 |
| 성능 | 스트리밍 응답 도입 (체감 응답 속도 개선) |
| 평가 | RAG 정확도 정량 평가 체계 구축 |
| 보안 | FastAPI 인증 추가 (API Key 또는 JWT) |
| 언어 전환 | 대화 중 명시적 언어 전환 요청 처리 |
