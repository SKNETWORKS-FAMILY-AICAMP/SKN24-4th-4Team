...
|------|------|
| 권민제 | |
| 김수진 | 기존 웹 개발 경험을 바탕으로 웹 파트를 빠르게 구축해 LLM 개발 단계까지 진입할 수 있었습니다. 이번 프로젝트를 진행하며 프로젝트 초기부터 의존성 버전을 requirements로 통일하고 웹–LLM 연동을 선행해 테스트를 진행함으로써 오류와 개선사항을 조기에 도출하고 고도화하는 과정이 중요하다고 느꼈습니다. |
...
| 김지원 | 비교 기능 구현 과정에서 구조화된 JSON 출력, 플랜 리스트 추출, ChromaDB 필터 문제 등을 해결하며 기존 텍스트 기반 응답과 다른 설계가 필요함을 깨달았다. 보험사 간 비교에서는 잘못된 라우팅과 언어 처리 문제를 수정하며 intent 기반 분기와 프롬프트 설계의 중요성을 확인했다. 환율 계산 기능 구현 과정에서는 API 변경에 따른 타입 불일치와 입력 조건 검증 문제를 겪으며, 인터페이스 관리와 예외 처리의 중요성을 배웠다.|
